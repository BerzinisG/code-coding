...
.then(res => res.json())
.then(zmones => { //sukuriam f-ja su kuria priskiriam duomenis kortelei
  console.log(zmones);
  const allCards = document.querySelector('#allCards');// priskiriam naują kintamajį id=allCards
  zmones.forEach(zmogus => {
    const zmogusDiv = new Zmogus(zmogus); // priskiriam klase kitam kintamajam, kurį priskirsim id=allCards
...
// 
  document.querySelector('#newCard > form').addEventListener('submit', e=>{
    ...
    // id: Date.now();
    console.log(zmogus);
    /// paimam iš sukurtos klasės Zmogus visus kortelės duomenis ir sukuriam koretele \\\
